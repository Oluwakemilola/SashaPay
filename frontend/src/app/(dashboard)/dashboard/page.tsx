...
import { Users, TrendingUp, CalendarCheck, Wallet, ShieldCheck, ArrowRight, Bot, Copy, RefreshCw, Check, PlayCircle } from "lucide-react";
...
function AdminDashboard({ dash, health, loading, error }: any) {
  ...
  // ── This is the key fix — actually creates a payroll run ──
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16, position: "relative" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
            <button onClick={handleRunPayroll} disabled={running}
              style={{ display: "flex", alignItems: "center", gap: 8, padding: "12px 24px", background: GOLD, color: "#fff", borderRadius: 12, fontSize: 14, fontWeight: 700, border: "none", cursor: running ? "not-allowed" : "pointer", fontFamily: "Outfit, sans-serif", opacity: running ? 0.7 : 1 }}>
              <PlayCircle style={{ width: 16, height: 16 }} />
              {running ? "Creating run..." : "Run Payroll"}
            </button>
            {runError && <p style={{ fontSize: 12, color: "#FCA5A5" }}>{runError}</p>}
          </div>
        </div>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #E8EDE8", padding: "20px 24px", marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 8 }}>
          ...
          <button onClick={handleRefresh} disabled={refreshing}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 14px", border: `1px solid ${GREEN}`, borderRadius: 8, background: "transparent", color: GREEN, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "Outfit, sans-serif" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          <button onClick={handleCopy}
            style={{ width: 48, height: 48, borderRadius: 10, border: `1px solid ${copied ? "#059669" : "#E8EDE8"}`, background: copied ? "#F0FDF4" : "#fff", color: copied ? "#059669" : "#9AADA6", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0 }}>
...
function WorkerDashboard({ att, pass, name, loading, error }: any) {
  ...
  const attendanceRate = att?.length > 0
    ? Math.round((att.filter((r: any) => r.status !== "ABSENT").length / att.length) * 100) : 0;
  const chartData = [...(att || [])].reverse().slice(-7).map((r: any) => ({
    day: new Date(r.date).toLocaleDateString("en-NG", { weekday: "short" }), hours: r.hoursWorked || 0,
  }));
  const greeting = () => {
    const h = new Date().getHours();
    return h < 12 ? "Good morning" : h < 17 ? "Good afternoon" : "Good evening";
    ...
    <div style={{ fontFamily: "Outfit, sans-serif" }}>
      <div style={{ background: GREEN, borderRadius: 20, padding: "28px 32px", marginBottom: 24, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16, position: "relative" }}>
          <div>
            <p style={{ color: "rgba(248,245,237,0.55)", fontSize: 13, marginBottom: 4 }}>{greeting()}, {firstName} 👋</p>
            <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 28, color: "#F8F5ED", lineHeight: 1.1, marginBottom: 4 }}>Your Financial Wellness</h2>
            <p style={{ color: "rgba(248,245,237,0.45)", fontSize: 13 }}>{new Date().toLocaleDateString("en-NG", { weekday: "long", month: "long", day: "numeric" })}</p>
          </div>
      ...
      {error && !loading && <div style={{ padding: "12px 16px", background: "#FEF2F2", color: "#DC2626", borderRadius: 10, marginBottom: 16, fontSize: 13 }}>⚠ {error}</div>}
      ...
      <div style={{ background: GREEN, borderRadius: 16, padding: "24px", marginBottom: 20, position: "relative", overflow: "hidden" }}>
        ...
        <h3 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 20, color: "#F8F5ED", marginBottom: 8 }}>Your Financial Passport</h3>
        <p style={{ fontSize: 13, color: "rgba(248,245,237,0.55)", marginBottom: 20, lineHeight: 1.6 }}>Every salary payment builds your verified financial identity. Use it with banks, landlords, and lenders.</p>
