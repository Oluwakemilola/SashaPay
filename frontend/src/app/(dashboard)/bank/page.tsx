...
const NIGERIAN_BANKS = [
  ...
  { name: "Union Bank",    code: "032" }, { name: "Stanbic IBTC",  code: "221" },
...
type BankAccount = {
  _id: string; bankName: string; bankCode: string;
  accountNumber: string; accountName: string;
  isPrimary: boolean; verificationStatus: string;
};
  ...
  return (
    <div style={{ fontFamily: "Outfit, sans-serif", maxWidth: 680 }}>
      <div style={{ marginBottom: 24 }}>
        <h2 style={{ fontFamily: "'DM Serif Display', serif", fontSize: 28, color: G, marginBottom: 4 }}>Bank Accounts</h2>
        <p style={{ fontSize: 14, color: "#6B7B72" }}>Add your bank account to receive salary payments. Your primary account is used for all disbursements.</p>
      </div>
            ...
            <div key={acc._id} style={{ background: "#fff", borderRadius: 14, border: `1.5px solid ${acc.isPrimary ? GOLD : "#E8EDE8"}`, padding: "18px 20px", display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3, flexWrap: "wrap" as const }}>
                  ...
                  {acc.isPrimary && <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 99, background: `${GOLD}20`, color: GOLD, textTransform: "uppercase" as const }}>Primary</span>}
              ...
              <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                {!acc.isPrimary && (
                  <>
                    <button onClick={() => handleSetPrimary(acc._id)} title="Set as primary" style={{ width: 32, height: 32, borderRadius: 8, border: `1px solid ${GOLD}`, background: "transparent", color: GOLD, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
                      <Star style={{ width: 14, height: 14 }} />
                    </button>
                    <button onClick={() => handleDelete(acc._id)} title="Remove" style={{ width: 32, height: 32, borderRadius: 8, border: "1px solid #FECACA", background: "transparent", color: "#DC2626", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #E8EDE8", padding: "24px", marginTop: 8 }}>
          ...
          <form onSubmit={handleAdd}>
            <div style={{ marginBottom: 16 }}>
              ...
              <select required value={form.bankCode} onChange={handleBankSelect} style={inputStyle}>
